            ...
            <div className="flex">
                <div className="mb-4 w-1/3 ">
                    {ratingLabels.map((item) => (
                        <div
                            key={item.star}
                            className={`flex flex-col items-center p-5 mb-2 cursor-pointer rounded-md ${rating === item.star ? 'bg-orange-100' : 'bg-gray-100'
                                }`}
                            onClick={() => setRating(item.star)}
                        >
                            <div className="flex gap-3 items-center">
                                <span className="text-sm">{item.star} Start</span>
                                <Rate value={item.star} disabled className="mr-2 mt-3" />
                            </div>

...
const ReviewModal: React.FC<ReviewModalProps> = ({ setShowModal }) => {
    ...
    return (
        <Modal
            ...
            footer={null}
            width={700} // Make modal wider to match design
            className="p-6"
        >
            <div className="flex">
                {/* Rating Section */}
                <div className="mb-4 w-1/3">
                    {ratingLabels.map((item) => (
                        <div
                            key={item.star}
                            className={`flex items-center justify-between p-3 mb-2 cursor-pointer rounded-md ${rating === item.star ? 'bg-orange-100' : 'bg-gray-100'
                                }`}
